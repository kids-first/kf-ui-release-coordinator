...
class PublishHistory extends Component {
  ...
  getLatest() {
    axios.get(`${coordinatorApi}/releases?state=published&limit=1000`)
         .then((releases) => {
            this.setState({
  ...
  render() {
    ...
    const data = this.state.publishedReleases.map((r, i) => (
          {x: new Date(r.created_at), y: i, l: r.version}
    ));
    return (
      <div>
        <VictoryChart
        ...
        <VictoryScatter
          ...
          />
          <VictoryAxis
            tickFormat={(x) => new Date(x).toLocaleString('en-us', { month: "short", day: 'numeric' })}
            style={{
              tickLabels: {
                fontSize: 6
              }
