...
class ReleaseTimeline extends Component {
  ...
  getLatest() {
    axios.get(`${coordinatorApi}/events?release=${this.props.releaseId}&limit=1000`)
         .then((events) => {
            this.setState({
  ...
  render() {
    ...
    const services = data.map((r) => r.name).filter((x, i, a) => a.indexOf(x) === i)
    ...
    return (
      <div>
        ...
        <VictoryScatter
          ...
          />
          <VictoryAxis
            tickFormat={(x) => new Date(x).toLocaleString('en-us', { month: "numeric", day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric'})}
            style={{
              tickLabels: {
                fontSize: 6,
                padding: 2,
